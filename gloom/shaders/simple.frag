...
float sd_scene(vec3 point)
{
    ...
    float sphere = sd_sphere(sd_translate(point, vec3(0.0f, sin(time) * 0.5f, 0.0f)), 0.45f);
...
{
    return sd_sphere(sd_translate(point, vec3(0.0f, sin(time) * 0.5f, 0.0f)), 0.45f);
}
...
vec3 phong_illumination(vec3 k_a, vec3 k_d, vec3 k_s, float alpha,
                        vec3 p, vec3 eye, vec3 light_intensity)
{
...
vec4 shade_scene()
{
    ...
    if (dist_plane < dist_ball) {
        ...
        if (ref_dist_ball < MAX_DIST - EPSILON) {
            ...
            vec4 reflection = vec4(phong_illumination(k_a, k_d, k_s, shininess, ball_p, p, ball_color), 1.0f);
            vec4 col = vec4(phong_illumination(k_a, k_d, k_s, shininess, p, eye, wave_color), 1.0f);
            return mix(col, reflection, fresnel);
        }

        return vec4(phong_illumination(k_a, k_d, k_s, shininess, p, eye, wave_color), 1.0f);
    } else {
        p = eye + dist_ball*dir;
        return vec4(phong_illumination(k_a, k_d, k_s, shininess, p, eye, ball_color), 1.0f);
    }
}
